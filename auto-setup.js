...
const fs_uniq = require('fs').promises;
let headers = {
...
async function manageMailAndRegister() {
    ...
    try {
        const typeKey = "2"; // Example hardcoded value for CAPTCHA API (Anti-Captcha)
        const apiKey = anti_apiKey[0]; // Replace this with the appropriate value or user input
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            const [email, password] = account.toString().split(":");
            const proxy = proxies[i]; // Ensure proxies are used as needed
            ...
            await delay(5000); // Add delay to avoid rate limiting or API blocking

            const referralCode = "DJNMDUID1PL5"; // Hardcoded referral code, can be replaced with user input

            try {
                logger(`Login account #${i + 1} - Email: ${email}`, 'debug');

                // Log in the user
                const loginData = await login(email, password, apiKey); // Pass the proxy if needed
                if (!loginData) {
                    logger(`Login failed for account #${i + 1}`, 'error');
        ...
        rl.close(); // Close the readline interface after completing the loop
    }
}
...
async function init(randomHex) {
    ...
    if (response.id) {
        try {
            ...
            await fs_uniq.appendFile('unique_id.txt', `${response.unique_id}\n`, 'utf-8');
